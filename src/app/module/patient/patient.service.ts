...
const updateMyProfile = async (
  ...
  await prisma.$transaction(async (tx) => {
    //update patient table with updated value
    ...
    if (payload.patientHealthData) {
      ...
      //if data already exist then update otherwise create
    ...
    //update reports if any
    if (
      payload.medicalReports &&
      Array.isArray(payload.medicalReports) &&
      payload.medicalReports.length > 0
    ) {
      //if - shouldDelete+report - true -> delete
      //if - reportname + reportLink - true -> create
